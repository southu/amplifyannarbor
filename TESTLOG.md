...
## Re-verification — run 063111 iter1 (2026-07-23)
...
- `GET /` → 200; `GET /donate` → 200 (donation form / Stripe checkout entry
  present); `GET /donate/success` → 200; `GET /version` → 200 (serves `d17c6d4`,
  == local HEAD).
- **AC9: no recurring tiers.** `/donate` presents one-time amounts only; no
  `monthly`/`recurring`/`interval`/`subscription` markers. No subscription was or
  should be created.
- Signature enforcement intact (NOT disabled to force AC6):
  `POST /api/webhooks/stripe` with bogus `Stripe-Signature` → 400; with no
  signature header → 400.
...
- **Operator secret store probed and LOCKED to the builder.** The Ratchet Vault
  (`VAULT_URL=http://127.0.0.1:8379`) is reachable (`/health` → 200) but its
  credential API is sealed to this run: `GET /api/items` → 401 `{"error":
  "locked"}`. No unlock session or reveal is available, so no `rk_live_`/`sk_live_`
  can be retrieved — and bypassing the lock is out of scope under the fail-closed
  constraint. The harness broker injected nothing this run:
  `RATCHET_PROVISION_ENV_FROM_VAULT_COUNT=0`, `RATCHET_PROVISION_ENABLED=false`.
  No builder-reachable path to a live Stripe key.
- **Cloudflare token — cannot remediate AC6.** `CLOUDFLARE_API_TOKEN` present but
  `GET /user/tokens/verify` → HTTP 401 `code 1000 Invalid API Token`. A Pages
  write grant would be moot anyway: rolling `STRIPE_WEBHOOK_SECRET` needs the new
  `whsec_` value, obtainable only from a Stripe write key (absent).
- Committed artifacts intact: `docs/stripe-cutover-runbook.md` (cutover steps,
  ...
  redacted; `webhook-event.json` still `pending_webhooks=1`,
  `webhook-delivery-status.json` still `delivered_2xx=false
  reason=signature_secret_mismatch`). **AC12 clean:** no full `(sk|rk)_live_` or
  `whsec_` key value anywhere in the committed tree (only prefix references in
  code/docs).
...
## Run 20260723-083005 — iteration 1 (builder re-verification, fail-closed)
...
- **Live site healthy:** `GET /` `/donate` `/donate/success` `/version` all 200;
  `/version` body `05a4b48c2214a330f7341c35a3fb7318e530c688` == HEAD before this
  commit. **AC9:** one-time amounts only, no recurring/subscription tiers.
- **Webhook sig verification intact:** `POST /api/webhooks/stripe` no-sig → 400,
  bogus-sig → 400 (fail-closed, not disabled to force AC6). Committed evidence
  still shows endpoint `we_1TwCAALA5oeiO5iDBwdOFWMt` `status=enabled`, redelivery
  `reason=signature_secret_mismatch` (BUG-1, `pending_webhooks=1`).
- **AC10 runbook intact:** `docs/stripe-cutover-runbook.md` still carries the
  `https://docs.stripe.com/projects` pointer. **AC12 clean:** length-bounded scan
  for real key material over the committed tree returns no matches.
