...
> **Blocker (important — why live price IDs are still pending):** Creating live-mode Stripe Price objects requires a **live-mode Stripe secret key**, and none is available in this build environment. Every credential source reachable here was checked this iteration and each carries only a **test-mode** key (or no Stripe key at all): the local build environment exposes no Stripe key variable; the Cloudflare Pages `amplifyannarbor` **production and preview** deployment configs both hold a test-mode secret key paired with a test-mode publishable key (so the live production site itself still runs Stripe in **test mode** and issues `cs_test_` sessions); and git history, the repo tree, and the run's shared directories contain no live key. Stripe API keys **cannot be minted programmatically** — they are issued only through an authenticated Stripe Dashboard/CLI login — so the builder cannot obtain one on its own. **`live_price_id` / `live_product_id` are therefore `null` for every tier.** No test-mode ID is presented as live and no ID is fabricated: a field named `live_price_id` must hold a genuine live-mode identifier or `null`, because a test-mode ID placed there would be false and would be **rejected by Stripe in live mode**, breaking real donation checkout at the cutover. The mapping fails safe (`null`, not wrong). The machine-readable copy (`public/stripe-cutover.json`) records this state under `live_objects_created: false` and a `blocker` section.

### 8.3 Why there are no sandbox price IDs, and why live IDs are still pending

- **Dynamic pricing, no stored objects — re-confirmed by object enumeration (2026-07-21).** As documented in §4/§6, the donate flow (`app/api/create-checkout/route.ts`) creates each Checkout Session with **dynamic `price_data`** (product *"Donation to Ann Arbor Meals on Wheels"*, `unit_amount = amount × 100`, currency `usd`). The account's Stripe objects were enumerated directly (`GET /v1/prices`, `/v1/products`, `/v1/payment_links`): **0 Prices, 0 Products, 0 Payment Links**. The sandbox has **zero persistent Stripe Price/Product objects and no static Payment Links**, so there are **no sandbox `price_…` identifiers to mirror** — `sandbox_price_id` is null for every tier because none has ever existed. For this site the real test→live cutover is an **environment-variable key swap** in Cloudflare Pages (see §6), plus creating the live Price objects below once a live key exists.
- **Live-mode key not available → live IDs pending (no fabrication).** Creating true live-mode Price objects requires a live-mode Stripe secret key. None is present or obtainable in this build environment (verified this iteration — see §8.2 blocker). An earlier iteration created real Price objects with the account's **test-mode** key to fill the mapping slots, but test-mode price IDs are a separate object space, are rejected by Stripe in live mode, and must not be published in a `live_price_id` field as though they were live — so those test IDs have been removed and `live_price_id`/`live_product_id` are `null` until a live key is provisioned. **No live IDs are fabricated and no test-mode ID is presented as live.** `scripts/create-live-stripe-prices.mjs` creates the live-mode equivalents the moment a live key is supplied.
